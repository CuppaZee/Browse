...
export const Sandbox = createBrowseModule(
  {
    name: "Refined Sandbox",
    id: "refined_sandbox",
    urls: ["*.munzee.com/map*"],
  },
  {
    async setupSandbox(a: number) {
      console.log(a)
      const { $ } = window as any;
      ...
      $("#showSBbuttons").click(function () {
        setTimeout(function () {
          $('[id*="SB"]').removeClass("hidden-xs");
          const { mapSandbox, map, onCameraChanged, mapboxgl, circle, quick_deploy } =
            window as any;
          ...
          mapSandbox.showItemPopup = function (item: any) {
            ...
            itemContent +=
              "<span><br />" + item.coordinates[1] + " " + item.coordinates[0] + "</span>";
  ...
  actions => {
    actions.setupSandbox(new BrowseModuleClass().x);
  }
);
